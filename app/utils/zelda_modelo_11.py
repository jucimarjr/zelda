...
class ZeldaModelo:
    ...
    @staticmethod
    def lista_processos():
        ZeldaModelo.processos.clear()
        for processo_id in db.get_processos_ids():
            processo = Processo(processo_id)
            ZeldaModelo.processos.append(processo)

        return ZeldaModelo.processos

    def lista_documentos():
        ZeldaModelo.documentos.clear()
        for documento_id in db.get_documentos_ids():
            documento = Documento(documento_id)
            ZeldaModelo.documentos.append(documento)

        return ZeldaModelo.documentos
